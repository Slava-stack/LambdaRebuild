const queries = {
  ...
  customerQuery:
    ...
    "WHERE CustomerID=?;",
  employeesQuery:
    'SELECT EmployeeID, CONCAT(LastName, " ", FirstName) AS FullName, Title, City, Country ' +
    "FROM employees2;",
  employeeQuery:
  ...
  ordersQuery:
    "SELECT O.OrderID, SUM(OD.UnitPrice * OD.Quantity) AS TotalPrice, COUNT(*) AS Products, " +
    ...
    "GROUP BY O.OrderID;",
  orderQuery1:
    "SELECT O.CustomerID, O.ShipName, COUNT(*) AS TotalProducts, " +
  ...
  productsQuery:
    ...
    "FROM products;",
  productQuery:
    "SELECT P.ProductID, P.ProductName, P.SupplierID, S.CompanyName, P.QuantityPerUnit, P.UnitPrice, P.UnitsInStock, P.UnitsOnOrder, " +
