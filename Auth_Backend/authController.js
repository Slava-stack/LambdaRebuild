...
class authController {
  async signUp(req, res) {
    try {
      ...
      if (!errors.isEmpty()) {
        return res
          .status(401)
          .json({ message: errors.errors.map((el) => `Error: ${el.msg}`) });
      }
      ...
      if (candidate) {
        return res
          .status(401)
          .json({ message: "A user with such email already exists" });
      }
  ...
  async signIn(req, res) {
    try {
      ...
      if (!user) {
        return res
          .status(401)
          .json({ message: `There is no user with the email ${email}` });
      }
  ...
  async getUser(req, res) {
    try {
      ...
      res
        .status(200)
        .json({ request_num, data: { username: `${email} ${password}` } });
    } catch (e) {
      console.log(e);
      return res.status(401).json({ message: "User is not authorized" });
    }
