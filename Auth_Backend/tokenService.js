...
class TokenService {
  generateTokens(payload) {
    ...
    const accessToken = jwt.sign(payload, secret_jwt_access, {
      expiresIn: random30or60sec,
    });
  ...
  async saveToken(userId, refreshToken) {
    try {
      ...
      if (!tokenData) {
        // this looks strange to me. Why do I even needed exclamation mark for updating? #15.12.2022
        return await refreshColl.findOneAndUpdate(
          { userId },
          { $set: { refreshToken } }
        );
      }
