...
const nameQuestion = () => {
  inquirer.prompt({
    type: "input", name: "name", message: "Enter user's name. To cancel press ENTER:",
  }).then(answer => (answer.name !== '') ? questionsContinuation(answer) : searcher());
}

const questionsContinuation = (objName) => {
  inquirer.prompt([{
    type: "list", name: "gender", message: "Choose your gender:", choices: ["male", "female"],
  }, {
    type: "input", name: "age", message: "Enter user's age:", validate: input => {
...
const searcher = () => {
  inquirer.prompt({
    type: "confirm", name: "searchInDB", message: "Would you like to search values in DB?",
  })
    .then(({ searchInDB }) => {
      if (searchInDB) {
...
const findByName = (objectsFromDB) => {
  inquirer.prompt({
    type: "input", name: "userName", message: "Enter user's name you wanna find in DB:"
  }).then(({ userName }) => {
    const nameArr = objectsFromDB.filter(el => (el.name.toLowerCase() === userName.toLowerCase()));
    ...
    console.log("There's no user with such name in DB");
  })
}
